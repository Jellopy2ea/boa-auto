// scraper.js - รันบน Github Actions ฟรี ไม่ต้องจ่าย Browser $5
    ...
    for (let opt = 1; opt <= 25; opt++) {
      try {
        ...
        if (pts.length) {
          ...
          if (nv > 1000 && nv < 500000) return nv;
        }
      } catch {}
    }
    return null;
...
async function main() {
  ...
  // อัพทีละ 12 ใบต่อรอบ เพื่อไม่ให้ Action รันนานเกิน 5 นาที
  const batch = keys.slice(0, 12);
  console.log('Batch', batch.length, 'of', keys.length);

  const browser = await chromium.launch({ headless: true });
  const page = await browser.newPage({ userAgent: 'Mozilla/5.0 (iPhone; CPU iPhone OS 17_0 like Mac OS X) AppleWebKit/605.1.15 Version/17.0 Mobile/15E148 Safari/604.1' });
  ...
  for (const k of batch) {
    ...
    try {
      ...
      if (p) {
        ...
        item.raw_jpy = p; item.jpy = p; item.raw_thb = Math.round(p * RATE); item.thb = Math.round(p * RATE); item.updated = new Date().toISOString();
        updatedCount++;
      } else console.log(k, 'no price');
    } catch (e) { console.log(k, 'error', e.message); }
    await new Promise(r => setTimeout(r, 1200));
  }
  await browser.close();
  if (updatedCount > 0) await putPrices(data);
